...
DEFAULT_EPOCHS = 40
DEFAULT_DRIVE_SCALES = [0.5, 1.0, 2.0]
VOLTAGE_AMP = 8.0
CURRENT_AMP = 1.0
V_OFF_WIDTH = 2.0
TRAIN_SUBSET = 128
...
def _prepare_config(*, init_drive_scale: float, device: str, epochs: int) -> dict:
    ...
    model_name = f"{cfg['model']['name']}_voff2p0_drive{drive_tag}_va8p0_bp"
...
def _run_one(*, init_drive_scale: float, device: str, epochs: int) -> dict:
    cfg = _prepare_config(init_drive_scale=init_drive_scale, device=device, epochs=epochs)
    start = time.time()
    ...
    print(
        f"[done] drive={init_drive_scale:g} train_best={row['train_best']:.4f} "
...
def _make_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Short BP-only init_drive_scale gate for the widened hard-sigmoid CIFAR config."
    )
    parser.add_argument("--device", default=("cuda" if torch.cuda.is_available() else "cpu"))
    parser.add_argument("--epochs", type=int, default=DEFAULT_EPOCHS)
    parser.add_argument("--drive-scales", default=",".join(str(v) for v in DEFAULT_DRIVE_SCALES))
    return parser
...
def main() -> int:
    ...
    drive_scales = _parse_values(args.drive_scales)
    rows = [_run_one(init_drive_scale=value, device=args.device, epochs=args.epochs) for value in drive_scales]
