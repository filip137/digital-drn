...
def _variant_name(block0_ff_lr: float, block1_ff_lr: float) -> str:
    left = f"{block0_ff_lr:.0e}".replace("+", "")
    right = f"{block1_ff_lr:.0e}".replace("+", "")
    return f"cifar10_wider_b0ff_{left}_b1ff_{right}"


def _lr_pairs() -> list[tuple[float, float]]:
    return [(left, right) for left in LR_VALUES for right in LR_VALUES]
...
def _base_config() -> dict:
    ...
    # Keep the selected DRN learning rates fixed at the best setting from the DRN sweep.
    config["model"]["blocks_config"][0]["drn"]["lr"] = 3.0e-4
    config["model"]["blocks_config"][1]["drn"]["lr"] = 3.0e-4

    return config


def _configure_variant(block0_ff_lr: float, block1_ff_lr: float) -> tuple[str, dict]:
    config = _base_config()
    variant_name = _variant_name(block0_ff_lr, block1_ff_lr)
    config["_selection"]["model"] = variant_name
    config["model"]["blocks_config"][0]["ff"]["lr"] = float(block0_ff_lr)
    config["model"]["blocks_config"][1]["ff"]["lr"] = float(block1_ff_lr)
...
def main() -> int:
    ...
    for block0_ff_lr, block1_ff_lr in _lr_pairs():
        variant_name, config = _configure_variant(block0_ff_lr, block1_ff_lr)
        print(
            f"[start] {variant_name} "
            f"block0_ff_lr={block0_ff_lr:g} block1_ff_lr={block1_ff_lr:g}"
        )
        ...
        try:
            ...
            row = {
                ...
                "block0_ff_lr": block0_ff_lr,
                "block1_ff_lr": block1_ff_lr,
                "block0_drn_lr": 3.0e-4,
                "block1_drn_lr": 3.0e-4,
